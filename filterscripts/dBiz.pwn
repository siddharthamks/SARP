...
#define     BUSINESS_DAYS  		(30)     // if a business's owner doesn't visit their business after x days, it will get reset (default: 7)
...
Player_ShowBusinessMenu(playerid)
{
	...
	Dialog_Show(playerid, DIALOG_MANAGE_BUSINESS, DIALOG_STYLE_TABLIST, "Business Management", string, "Choose", "Cancel");
	return 1;
}
...
Player_ShowBusinessSafe(playerid)
{
	...
	Dialog_Show(playerid, DIALOG_MANAGE_BUSINESS_SAFE, DIALOG_STYLE_TABLIST, "Business Management: Safe", string, "Choose", "Cancel");
	return 1;
}
...
Player_ShowBusinessSale(playerid)
{
    ...
    format(string, sizeof(string), "Sell Instantly\t{2ECC71}$%s\n%s", convertNumber(floatround(BusinessData[id][Price] * 0.85)), (BusinessData[id][SalePrice] > 0) ? ("Remove From Sale") : ("Put On Sale"));
   	Dialog_Show(playerid, DIALOG_MANAGE_SELL, DIALOG_STYLE_TABLIST, "Business Management: Sell", string, "Choose", "Cancel");
	return 1;
}

Player_ShowPermissionMenu(playerid)
{
    Dialog_Show(playerid, DIALOG_MANAGE_PERMISSIONS, DIALOG_STYLE_TABLIST, "Business Management: Permissions", "Give Permissions\nPlayers With Permissions\nRemove All Permissions", "Choose", "Cancel");
	return 1;
}
...
Player_ShowPermissionEditing(playerid)
{
	...
	if(cache_num_rows() > 0) {
		...
		Dialog_Show(playerid, DIALOG_MANAGE_EDIT_PERMS, DIALOG_STYLE_TABLIST, "Business Management: Edit Permissions", string, "Choose", "Cancel");
	}else{
...
Player_ShowSafeLogs(playerid)
{
	...
	if(rows) {
		...
		Dialog_Show(playerid, DIALOG_MANAGE_SAFE_LOGS, DIALOG_STYLE_TABLIST_HEADERS, title, list, "Next", "Previous");
	}else{
...
public BusinessSaleMoney(playerid)
{
    new rows = cache_num_rows();
 	if(rows)
  	{
   		new new_owner[MAX_PLAYER_NAME], amount, string[128];
		for(new i; i < rows; i++)
		{
	    	cache_get_field_content(i, "NewOwner", new_owner);
		    amount = cache_get_field_content_int(i, "Amount");

			format(string, sizeof(string), "You sold a business to %s for $%s. (Transaction ID: #%d)", new_owner, convertNumber(amount), cache_get_field_content_int(i, "ID"));
...
public OnFilterScriptInit()
{
	...
	BusinessSQL = mysql_connect(SQL_HOST, SQL_USER, SQL_DBNAME, SQL_PASSWORD);
				...
				if(pickupid == BusinessData[i][BusinessPickup])
				{
			        ...
			        if(!strcmp(BusinessData[i][Owner], "-") || BusinessData[i][SalePrice] > 0) {
			            ...
			            if(BusinessData[i][SalePrice] > 0) {
			                ...
			                Dialog_Show(playerid, DIALOG_BUY_BUSINESS_FROM_OWNER, DIALOG_STYLE_MSGBOX, "Business For Sale", string, "Buy", "Cancel");
						}else{
			                format(string, sizeof(string), "This business is for sale.\nPrice: {2ECC71}$%s", convertNumber(BusinessData[i][Price]));
			                Dialog_Show(playerid, DIALOG_BUY_BUSINESS, DIALOG_STYLE_MSGBOX, "Business For Sale", string, "Buy", "Cancel");
			            }
			        }else{
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch(dialogid)
	{
		...
		case DIALOG_MANAGE_BUSINESS:
		{
		    ...
		    if(listitem == 0)
		    {
		        ...
		        format(string, sizeof(string), "Current Business Name: %s\n\n{FFFFFF}Write your new business name:", BusinessData[ InBusiness[playerid] ][Name]);
     	        Dialog_Show(playerid, DIALOG_MANAGE_BUSINESS_NAME, DIALOG_STYLE_INPUT, "Business Management: Name", string, "Change", "Cancel");
		    }
		...
		case DIALOG_MANAGE_BUSINESS_NAME:
		{
		    ...
		    if(InBusiness[playerid] == INVALID_BUSINESS) return SendClientMessage(playerid, 0xE74C3CFF, "ERROR: {FFFFFF}You're not in a business.");
		    if(!Player_OwnsBusiness(playerid, InBusiness[playerid])) return SendClientMessage(playerid, 0xE74C3CFF, "ERROR: {FFFFFF}You don't own this business.");
		    if(!(1 <= strlen(inputtext) <= MAX_BUSINESS_NAME)) return Dialog_Show(playerid, DIALOG_MANAGE_BUSINESS_NAME, DIALOG_STYLE_INPUT, "Business Management: Name", "{E74C3C}ERROR: {FFFFFF}The business name you entered is either too short or too long.", "Change", "Cancel");
			new id = InBusiness[playerid];
        ...
        case DIALOG_MANAGE_BUSINESS_SAFE:
		{
			...
			if(listitem == 0)
			{
				...
				Dialog_Show(playerid, DIALOG_MANAGE_DEPOSIT_TO_SAFE, DIALOG_STYLE_INPUT, "Business Safe: Deposit", string, "Deposit", "Cancel");
			}
			...
			if(listitem == 1)
			{
				...
				Dialog_Show(playerid, DIALOG_MANAGE_TAKE_FROM_SAFE, DIALOG_STYLE_INPUT, "Business Safe: Take", string, "Take", "Cancel");
			}
		...
		case DIALOG_MANAGE_DEPOSIT_TO_SAFE:
		{
		    ...
		    if(BusinessData[ InBusiness[playerid] ][SalePrice] > 0) return SendClientMessage(playerid, 0xE74C3CFF, "ERROR: {FFFFFF}You can't use the safe while the business is on sale.");
		    if(!Player_CheckPermission(playerid, InBusiness[playerid], PERM_CAN_DEPOSIT)) return SendClientMessage(playerid, 0xE74C3CFF, "ERROR: {FFFFFF}You can't deposit money to this business.");
      		if(!(1 <= strval(inputtext) <= 100000000)) return Dialog_Show(playerid, DIALOG_MANAGE_DEPOSIT_TO_SAFE, DIALOG_STYLE_INPUT, "Business Safe: Deposit", "{E74C3C}ERROR: {FFFFFF}You can't deposit less than $1 or more than $100.000.000 at once.", "Deposit", "Cancel");
			if(strval(inputtext) > GetPlayerMoney(playerid)) return Dialog_Show(playerid, DIALOG_MANAGE_DEPOSIT_TO_SAFE, DIALOG_STYLE_INPUT, "Business Safe: Deposit", "{E74C3C}ERROR: {FFFFFF}You don't have that much money on you.", "Deposit", "Cancel");
		...
		case DIALOG_MANAGE_TAKE_FROM_SAFE:
		{
		    ...
		    if(BusinessData[ InBusiness[playerid] ][SalePrice] > 0) return SendClientMessage(playerid, 0xE74C3CFF, "ERROR: {FFFFFF}You can't use the safe while the business is on sale.");
		    if(!Player_CheckPermission(playerid, InBusiness[playerid], PERM_CAN_TAKE)) return SendClientMessage(playerid, 0xE74C3CFF, "ERROR: {FFFFFF}You can't take money from this business.");
      		if(!(1 <= strval(inputtext) <= 100000000)) return Dialog_Show(playerid, DIALOG_MANAGE_TAKE_FROM_SAFE, DIALOG_STYLE_INPUT, "Business Safe: Take", "{E74C3C}ERROR: {FFFFFF}You can't take less than $1 or more than $100.000.000 at once.", "Take", "Cancel");
			if(strval(inputtext) > BusinessData[ InBusiness[playerid] ][Money]) return Dialog_Show(playerid, DIALOG_MANAGE_TAKE_FROM_SAFE, DIALOG_STYLE_INPUT, "Business Safe: Take", "{E74C3C}ERROR: {FFFFFF}This business don't have that much money.", "Take", "Cancel");
			new id = InBusiness[playerid], amount = strval(inputtext);
			GivePlayerMoney(playerid, amount);
			BusinessData[id][Money] -= amount;
        	BusinessData[id][Save] = true;
		...
		case DIALOG_MANAGE_SELL:
		{
			...
			if(listitem == 1)
			{
				if(BusinessData[id][SalePrice] > 0) {
					...
					Dialog_Show(playerid, DIALOG_MANAGE_SELL_TO_PLAYERS, DIALOG_STYLE_INPUT, "Business Management: Sell", "How much do you want for your business?", "Put On Sale", "Cancel");
				}
			}
		...
		case DIALOG_MANAGE_SELL_TO_PLAYERS:
		{
		    ...
		    if(InBusiness[playerid] == INVALID_BUSINESS) return SendClientMessage(playerid, 0xE74C3CFF, "ERROR: {FFFFFF}You're not in a business.");
		    if(!Player_OwnsBusiness(playerid, InBusiness[playerid])) return SendClientMessage(playerid, 0xE74C3CFF, "ERROR: {FFFFFF}You don't own this business.");
		    if(!(1 <= strval(inputtext) <= 100000000)) return Dialog_Show(playerid, DIALOG_MANAGE_SELL_TO_PLAYERS, DIALOG_STYLE_INPUT, "Business Management: Sell", "{E74C3C}ERROR: {FFFFFF}You can't put your business on sale for less than $1 or more than $100.000.000.", "Put On Sale", "Cancel");
			new id = InBusiness[playerid], string[128];
		...
		case DIALOG_MANAGE_PERMISSIONS:
		{
		    ...
		    if(!Player_OwnsBusiness(playerid, InBusiness[playerid])) return SendClientMessage(playerid, 0xE74C3CFF, "ERROR: {FFFFFF}You don't own this business.");
			new id = InBusiness[playerid];
			if(listitem == 0) Dialog_Show(playerid, DIALOG_MANAGE_GIVE_PERM_NAME, DIALOG_STYLE_INPUT, "Business Management: Give Permission", "Write the player's name:", "Continue", "Cancel");
			if(listitem == 1)
			{
				...
				if(rows > 0) {
					...
					Dialog_Show(playerid, DIALOG_MANAGE_LIST_PERMS, DIALOG_STYLE_TABLIST_HEADERS, "Business Management: Permissions", string, "Edit", "Cancel");
				}else{
		...
		case DIALOG_MANAGE_GIVE_PERM_NAME:
		{
		    ...
		    if(InBusiness[playerid] == INVALID_BUSINESS) return SendClientMessage(playerid, 0xE74C3CFF, "ERROR: {FFFFFF}You're not in a business.");
		    if(!Player_OwnsBusiness(playerid, InBusiness[playerid])) return SendClientMessage(playerid, 0xE74C3CFF, "ERROR: {FFFFFF}You don't own this business.");
		    if(!(1 <= strlen(inputtext) <= MAX_PLAYER_NAME)) return Dialog_Show(playerid, DIALOG_MANAGE_GIVE_PERM_NAME, DIALOG_STYLE_INPUT, "Business Management: Give Permission", "{E74C3C}ERROR: {FFFFFF}The player name you entered is either too short or too long.", "Continue", "Cancel");
			...
			Dialog_Show(playerid, DIALOG_MANAGE_GIVE_PERM_DEPOSIT, DIALOG_STYLE_TABLIST_HEADERS, "Business Management: Give Permission", "Can this player deposit money in the business safe?\nNo\nYes", "Continue", "Cancel");
		    return 1;
		}
		...
		case DIALOG_MANAGE_GIVE_PERM_DEPOSIT:
		{
			...
			Dialog_Show(playerid, DIALOG_MANAGE_GIVE_PERM_TAKE, DIALOG_STYLE_TABLIST_HEADERS, "Business Management: Give Permission", "Can this player take money from the business safe?\nNo\nYes", "Finish", "Cancel");
		    return 1;
		}
...
CMD:setbusinesstype(playerid, params[])
{
	...
	if(type == -1) {
		...
		Dialog_Show(playerid, DIALOG_ADMIN_BUSINESS_TYPE, DIALOG_STYLE_TABLIST_HEADERS, "Business Types", string, "Set", "Cancel");
